...
import { Calendar, Trophy, Sparkles, LogOut, BarChart3, ClipboardList } from 'lucide-react-native';
import * as Haptics from 'expo-haptics';
import { useAuth } from '@/contexts/AuthContext';
import { Colors } from '@/constants/colors';
import { getRoleLabel, getInitials } from '@/types';

export default function TeamLeadMoreScreen() {
  const { user, logoutMutation } = useAuth();
  const router = useRouter();

  const menuItems = [
    { label: 'AI Analyses', icon: BarChart3, route: '/analyses', color: Colors.green },
    { label: 'My Daily Log', icon: ClipboardList, route: '/teamlead-log', color: Colors.orange },
      ...
      <SafeAreaView edges={['top']} style={styles.safeArea}>
        <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
          <View style={styles.profileSection}>
            ...
            <Text style={styles.role}>{getRoleLabel(user?.role ?? 'teamlead')}</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 64, height: 64, borderRadius: 32, backgroundColor: Colors.pink, justifyContent: 'center', alignItems: 'center', marginBottom: 12 },
