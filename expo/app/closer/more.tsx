...
import { PhoneOff, Calendar, Trophy, Sparkles, LogOut, KeyRound, Sun, Moon } from 'lucide-react-native';
...
export default function CloserMoreScreen() {
  ...
  const menuItems = [
    { label: 'No Answer Log', icon: PhoneOff, route: '/no-answer', color: colors.orange },
      ...
      <SafeAreaView edges={['top']} style={styles.safeArea}>
        <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
          <View style={styles.profileSection}>
            ...
            <Text style={[styles.role, { color: colors.muted }]}>{getRoleLabel(user?.role ?? 'closer')}</Text>
            <Text style={[styles.empId, { color: colors.muted }]}>ID: {user?.id ? mapSupabaseIdToEmployeeId(user.id) : ''}</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  logoutBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255,107,107,0.1)',
    borderRadius: 12, padding: 16, marginTop: 30, gap: 8,
  },
